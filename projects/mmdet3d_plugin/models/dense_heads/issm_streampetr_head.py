...
class ISSMStreamPETRHead(AnchorFreeHead): 
    def __init__(
        ...
        **kwargs
        ...
        # 在 super().__init__() 之前用普通变量保存 pc_range（不能用 nn.Parameter）
        # 因为 _init_layers() 会在 super().__init__() 中被调用
        self._bbox_coder_cfg = bbox_coder
        self._pc_range_list = build_bbox_coder(bbox_coder).pc_range
        
        super(ISSMStreamPETRHead, self).__init__(num_classes, in_channels, init_cfg=kwargs.get('init_cfg', None))
        
        # super().__init__() 之后创建 nn.Parameter
        ...
        # bbox_coder 和 pc_range 已在 super().__init__() 之前设置
    ...
    def _init_layers(self):
        ...
        self.query_memory_attn_layers = nn.ModuleList([
        ...
        self.query_memory_norm_layers = nn.ModuleList([
            nn.LayerNorm(self.embed_dims)
            for _ in range(self.num_pred)
        ])

        if self.use_issm:
            # 使用 super().__init__() 之前保存的 _pc_range_list
            pc_range_list = list(self._pc_range_list)
    ...
    def pre_update_memory(self, data):
        ...
        if self.memory_embedding is None:
            ...
            # 场景切换时清空 memory（只保留 memory_len 个）
    ...
    def forward(self, memory_center, img_metas, **data):
        ...
        # === 5. 初始化 Query（包含时序对齐）===
        queries, query_pos, reference_points, temp_memory, temp_pos, rec_ego_pose = self._init_queries(B, img_metas, **data)
        ...
        for layer_idx in range(self.num_pred):
            ...
            if temp_memory is not None and temp_memory.size(1) > 0:
                # 构造 K/V：当前层 Query + 历史 Memory
                kv_content = torch.cat([query_out, temp_memory], dim=1)  # [B, 256 + N_mem, D]
                kv_pos = torch.cat([curr_query_pos, temp_pos], dim=1)    # [B, 256 + N_mem, D]
                
                # Self-Attention: Q = query + pos, K/V = (query + memory) + pos
                q = query_out + curr_query_pos
                k = kv_content + kv_pos
                v = kv_content  # Value 不加位置编码（StreamPETR 风格）
                
                # 执行 Self-Attention
                attn_output, _ = self.query_memory_attn_layers[layer_idx](q, k, v)
                query_out = self.query_memory_norm_layers[layer_idx](query_out + attn_output)
        ...
        if self.training:
            ...
            final_reference_points_denorm = (
                init_reference_points * (self.pc_range[3:6] - self.pc_range[0:3]) + 
                self.pc_range[0:3]
            )
    ...
    def temporal_alignment(self, query_pos, tgt, reference_points):
        ...
        Returns:
            ...
            temp_memory: [B, memory_len - N_prop, D] 剩余历史 Memory（用于 Self-Attention）
            temp_pos: [B, memory_len - N_prop, D] 剩余历史位置编码
            rec_ego_pose: [B, N_q + N_prop, 4, 4] ego pose 矩阵
        """
        B = query_pos.size(0)

        # === 1. 处理全部历史 Memory（memory_len 个）===
        # 归一化 reference points
        ...
        temp_pos += self.time_embedding(pos2posemb1d(self.memory_timestamp).float())  # 历史帧 t<0

        # === 4. 【StreamPETR 风格】分离传播 query 和剩余 Memory ===
        if self.num_propagated > 0:
            # 前 num_propagated 个作为传播 query，拼接到当前 query
            ...
            # 剩余的 Memory 用于 Self-Attention（不参与 query 拼接）
    ...
    def _init_queries(self, B, img_metas, **data):
        """       
        【StreamPETR 风格】Query 初始化：准备 Query 和历史 Memory 供后续每层 Self-Attention 使用
        ...
        Returns:
            ...
            query_pos: [B, N_q + N_prop, D] Query 位置编码（用于 ISSM 位置交互）
            reference_points: [B, N_q + N_prop, 3] Query 的 3D reference points
            temp_memory: [B, N_mem, D] 历史 Memory（用于每层 Self-Attention）
            temp_pos: [B, N_mem, D] 历史 Memory 位置编码
        ...
        # 【修改】temporal_alignment 现在返回 temp_memory 和 temp_pos
        ...
        # 返回 temp_memory 和 temp_pos 供 forward 中每层 Self-Attention 使用
        return queries, query_pos, reference_points_result, temp_memory, temp_pos, rec_ego_pose

    def _post_update_memory(self, queries, anchors, cls_scores, rec_ego_pose, 
                           all_bbox_preds, outs_dec, **data):
        """
        【StreamPETR 风格】训练后更新 Memory Queue（累积式）
        
        设计说明：
        1. 每帧选择 topk_proposals 个高置信度 query
        2. 拼接到 Memory 队列前面（新的在前）
        3. Memory 最多保留 memory_len 个（约 4 帧历史）
        4. 下一帧从 Memory 前 num_propagated 个取出传播
        """
        # 获取最后一层的预测结果
        ...
        # === 【StreamPETR 风格】选择 topk_proposals 个高置信度 query ===
        ...
        # === 【StreamPETR 风格】累积式 Memory 更新 ===
        # 新的 query 拼接到前面，旧的在后面
